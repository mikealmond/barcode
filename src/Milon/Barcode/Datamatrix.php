...
// -----------------------------------------------------------------------------
     ...
     * @protected
     */
    protected function getGFProduct($a, $b, $log, $alog, $gf) : int
    {
        if ($a == 0 or $b == 0) {
            return 0;
        }
     ...
     * @param int   $gf Number of fields on log/antilog table (power of 2).
     * @param int   $pp The value of its prime modulus polynomial (301 for ECC200).
     *
     * @return array data codewords + error codewords
     * @protected
     */
    protected function getErrorCorrection($wd, $nb, $nd, $nc, $gf = 256, $pp = 301) : array
    {
     ...
     * @protected
     */
    protected function get255StateCodeword($cwpad, $cwpos) : int
    {
     ...
     * @protected
     */
    protected function lookAheadTest($data, $pos, $mode) : int
    {
     ...
     * @protected
     */
    protected function getSwitchEncodingCodeword($mode) : int
    {
        switch ($mode) {
            case ENC_ASCII:
            { // ASCII character 0 to 127
     ...
     * @protected
     */
    protected function getMaxDataCodewords($numcw) : int
    {
        ...
        while ($pos < $data_length) {
            ...
            switch ($enc) {
                ...
                { // STEP B. While in ASCII encodation
                    if ($data_length > 1 and $pos < $data_length - 1 and $this->isCharMode(ord($data[$pos]),
                                ENC_ASCII_NUM) and $this->isCharMode(ord($data[$pos + 1]), ENC_ASCII_NUM)) {
     ...
     * @protected
     */
    protected function placeModule($marr, $nrow, $ncol, $row, $col, $chr, $bit) : array
    {
        if ($row < 0) {
            $row += $nrow;
            $col += 4 - ($nrow + 4) % 8;
     ...
     * @protected
     */
    protected function placeUtah($marr, $nrow, $ncol, $row, $col, $chr) : array
    {
     ...
     * @protected
     */
    protected function placeCornerA($marr, $nrow, $ncol, $chr) : array
    {
     ...
     * @protected
     */
    protected function placeCornerB($marr, $nrow, $ncol, $chr) : array
    {
     ...
     * @protected
     */
    protected function placeCornerC($marr, $nrow, $ncol, $chr) : array
    {
        $marr = $this->placeModule($marr, $nrow, $ncol, $nrow - 3, 0, $chr, 1);
        $marr = $this->placeModule($marr, $nrow, $ncol, $nrow - 2, 0, $chr, 2);
        $marr = $this->placeModule($marr, $nrow, $ncol, $nrow - 1, 0, $chr, 3);
     ...
     * @protected
     */
    protected function placeCornerD($marr, $nrow, $ncol, $chr) : array
    {
        $marr = $this->placeModule($marr, $nrow, $ncol, $nrow - 1, 0, $chr, 1);
     ...
     * @protected
     */
    protected function getPlacementMap($nrow, $ncol) : array
    {
